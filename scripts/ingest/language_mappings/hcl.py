...
class HclMapping(BaseMapping):
    ...
    def get_query_for_concept(self, concept: ConceptType) -> str | None:
        ...
        if concept == ConceptType.BLOCK:
            # Only use explicit HCL blocks as containers (avoid noisy body/object captures)
            return """
            (block) @definition
            """

        if concept == ConceptType.COMMENT:
            return """
            (comment) @definition
            """

        if concept == ConceptType.STRUCTURE:
            return """
    ...
    def extract_name(
        self, concept: ConceptType, captures: dict[str, Node], content: bytes
    ) -> str:
    ...
    def extract_constants(
        ...
        Args:
            concept: The ConceptType being extracted
